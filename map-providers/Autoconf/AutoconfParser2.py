...
class AutoconfParser(XmlParser):
    ...
    def parser_loop(self):
        ...
        if self._xml_parser.readNext() != QXmlStreamReader.StartDocument:
            self._raise()
        if not self._read_match_start_element(root_tag):
            self._raise()
        ...
        while not self._read_match_end_element(root_tag):
            if self._xml_parser.isStartElement():
                ...
                #     layers = self._parse_LayerList()
                    ...
                    # self._raise()
                    self._handle_unknown_element(ename)
            # else
            # or match end and
        ...
        return Autoconf(contents) # general, layers
    ...
    def _parse_Contents(self):
        ...
        layers = []
        tile_matrix_sets = []
        while not self._read_match_end_element('Contents'):
            if self._xml_parser.isStartElement():
                ename = self._xml_parser.name()
                if ename == 'Layer':
                    layers.append(self._parse_Layer())
                elif ename == 'TileMatrixSet':
                    tile_matrix_sets.append(self._parse_TileMatrixSet())
                else:
                    # self._raise()
                    print("Warning: unknown element", ename)
                    self._read_until_end_of(ename)

        self.pop()

        return Contents(layers, tile_matrix_sets)
    ...
    def _parse_WGS84BoundingBox(self):
      ...
      # </ows:WGS84BoundingBox>
        ...
        if not self._match_start_element('WGS84BoundingBox'):
            self._raise()

        while not self._read_match_end_element('WGS84BoundingBox'):
            if self._match_empty():
                continue
            if self._xml_parser.isStartElement:
                ename = self._xml_parser.name()
                if ename == 'LowerCorner':
                    x_min, y_min = self._read_float_list(sep=' ')
                elif ename == 'UpperCorner':
                    x_max, y_max = self._read_float_list(sep=' ')
                else:
                    self._handle_unknown_element(ename)
                    # self._raise()

        return BoundingBox('wgs84', x_min, y_min, x_max, y_max)
        ...
        while not self._read_match_end_element('TileMatrix'):
            ...
            if self._xml_parser.isStartElement():
                ...
                if ename == 'Identifier':
                    ...
                    top_left_corner = self._read_float_list(sep=' ')
    ...
    def _parse_Layer(self):
        ...
        while not self._read_match_end_element('Layer'):
            ...
            if self._xml_parser.isStartElement():
                ...
                elif ename == 'WGS84BoundingBox':
                    bounding_box = self._parse_WGS84BoundingBox()
                elif ename == 'Identifier':
                    identifier = self._read_text()
                elif ename == 'InfoFormat':
                    info_format = self._read_text()
                elif ename == 'Format':
                    format = self._read_text()
                elif ename == 'Style':
    ...
    def _parse_Dimension(self):
        ...
        return Dimension(name, unit_symbol, units, user_value, value)
