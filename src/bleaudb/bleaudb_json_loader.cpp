...
load_json_bleaudb(const QString & json_path, BleauDB & bleaudb)
{
  ...
  for (const QJsonValue & json_value : json_places) {
    ...
    place.set_coordinate(QGeoCoordinate(latitude, longitude));
    place.set_name(json_object[QStringLiteral("name")].toString());
    // place.set_category(json_object[QStringLiteral("category")].toString());
    place.set_note(json_object[QStringLiteral("note")].toString());
    bleaudb.append_place(place);
  }
  ...
  for (const QJsonValue & json_value : json_massifs) {
    ...
    double longitude = json_object[QStringLiteral("longitude")].toDouble();
    double latitude = json_object[QStringLiteral("latitude")].toDouble();
    massif.set_coordinate(QGeoCoordinate(latitude, longitude));
    massif.set_name(json_object[QStringLiteral("name")].toString());
    ...
    massif.set_note(json_object[QStringLiteral("note")].toString());
  ...
  for (const QJsonValue & json_value : json_circuits) {
    ...
    double longitude = json_object[QStringLiteral("longitude")].toDouble();
    double latitude = json_object[QStringLiteral("latitude")].toDouble();
    circuit.set_coordinate(QGeoCoordinate(latitude, longitude));
    ...
    // circuit.set_grade(json_object[QStringLiteral("grade")].toString());
    // circuit.set_massif(json_object[QStringLiteral("massif")].toString());
    circuit.set_note(json_object[QStringLiteral("note")].toString());
    circuit.set_number(json_object[QStringLiteral("number")].toInt());
    ...
    // circuit.set_topos(json_object[QStringLiteral("topos")].toString());
    ...
    for (const QJsonValue & json_value : json_boulders) {
      ...
      double latitude = json_object[QStringLiteral("latitude")].toDouble();
      circuit.set_coordinate(QGeoCoordinate(latitude, longitude));
      boulder.set_name(json_object[QStringLiteral("name")].toString());
      boulder.set_comment(json_object[QStringLiteral("comment")].toString());
      // boulder.set_grade(json_object[QStringLiteral("grade")].toString());
      // boulder.set_number(json_object[QStringLiteral("number")].toString());
      // boulders.append(boulder);
      circuit.append_boulder(boulder);
    }
    // circuit.set_boulders(boulders);

    bleaudb.append_circuit(circuit);
  }
}
