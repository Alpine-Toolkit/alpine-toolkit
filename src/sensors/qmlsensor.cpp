...
QTQmlSensor::QTQmlSensor(QObject *parent)
    : QObject(parent)
    , m_parsed(false)
    , m_active(false)
    , m_reading(0)
...
QString QTQmlSensor::identifier() const
{
    return m_identifier;
}
...
void QTQmlSensor::setActive(bool active)
{
    m_active = active;
...
bool QTQmlSensor::isActive() const
{
    return m_active;
}
...
QQmlListProperty<QTQmlSensorRange> QTQmlSensor::availableDataRates() const
{
    QList<QTQmlSensorRange*> ret;
    ret.reserve(sensor()->availableDataRates().size());
    foreach (const qrange &r, sensor()->availableDataRates()) {
        QTQmlSensorRange *range = new QTQmlSensorRange;
...
int QTQmlSensor::dataRate() const
{
    return sensor()->dataRate();
}

void QTQmlSensor::setDataRate(int rate)
{
    if (rate != dataRate()) {
...
QQmlListProperty<QTQmlSensorOutputRange> QTQmlSensor::outputRanges() const
{
    QList<QTQmlSensorOutputRange*> ret;
    ret.reserve(sensor()->outputRanges().size());
    foreach (const qoutputrange &r, sensor()->outputRanges()) {
        QTQmlSensorOutputRange *range = new QTQmlSensorOutputRange;
        //QQmlEngine::setObjectOwnership(range, QQmlEngine::JavaScriptOwnership);
...
int QTQmlSensor::outputRange() const
{
    return sensor()->outputRange();
}

void QTQmlSensor::setOutputRange(int index)
{
...
QString QTQmlSensor::description() const
{
...
QTQmlSensorReading *QTQmlSensor::reading() const
{
    return m_reading;
}
...
QTQmlSensor::AxesOrientationMode QTQmlSensor::axesOrientationMode() const
{
    return static_cast<QTQmlSensor::AxesOrientationMode>(sensor()->axesOrientationMode());
}

void QTQmlSensor::setAxesOrientationMode(QTQmlSensor::AxesOrientationMode axesOrientationMode)
{
...
int QTQmlSensor::currentOrientation() const
{
...
int QTQmlSensor::userOrientation() const
{
    return sensor()->userOrientation();
}

void QTQmlSensor::setUserOrientation(int userOrientation)
{
...
int QTQmlSensor::maxBufferSize() const
{
...
int QTQmlSensor::efficientBufferSize() const
{
...
int QTQmlSensor::bufferSize() const
{
    return sensor()->bufferSize();
}

void QTQmlSensor::setBufferSize(int bufferSize)
{
...
void QTQmlSensor::stop()
{
    setActive(false);
}

void QTQmlSensor::classBegin()
{
}

void QTQmlSensor::componentComplete()
{
    m_parsed = true;
...
void QTQmlSensor::_update()
{
}

void QTQmlSensor::updateReading()
{
    if (m_reading) {
        m_reading->update();
...
/*!
    ...
    \instantiates QTQmlSensorReading
    \inqmlmodule QtSensors
    \since QtSensors 5.0
...
QTQmlSensorReading::QTQmlSensorReading(QSensor *)
    : QObject(0)
{
}

QTQmlSensorReading::~QTQmlSensorReading()
{
}
...
quint64 QTQmlSensorReading::timestamp() const
{
    return m_timestamp;
}

void QTQmlSensorReading::update()
{
